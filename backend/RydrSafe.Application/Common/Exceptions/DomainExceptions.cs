...
/// <summary>
/// Thrown when a signed-in user re-enters their password to confirm a sensitive action and gets
/// it wrong. Deliberately not <see cref="UnauthorizedAccessException"/>: that surfaces as 401,
/// which the client treats as an expired session and answers by signing the user out. Mistyping
/// a confirmation password should cost you an error message, not your session.
/// </summary>
public class CredentialConfirmationException(string message) : Exception(message);
